...
<body>
  ...
  <script>
    ...
    function submitData(status) {
      ...
      if (!name || !program || !purpose) {
        alert("Please fill in all fields.");
        return;
      }

      const entry = { date, time, name, program, purpose, status };
    ...
    function loadLogs() {
      ...
      logs.forEach(log => {
        ...
        row.innerHTML = `<td>${log.date}</td><td>${log.time}</td><td>${log.name}</td><td>${log.program}</td><td>${log.purpose}</td><td>${log.status}</td>`;
